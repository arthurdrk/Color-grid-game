...
class SolverHungarian(Solver):
    ...
    def linear_sum_assignment(self, cost, maximize=False):
        ...
        u = np.full(cost.shape[0], 0., dtype=float)
        v = np.full(cost.shape[1], 0., dtype=float)
        ...
        for current_row in range(cost.shape[0]):
            cost, u, v, path, row4col, col4row = self._lsa_body(cost, u, v, path, row4col, col4row, current_row)
    ...
    def _find_short_augpath_while_body_inner_for(self, it, val):
        ...
        if (shortest_path_costs[j] < lowest) or (shortest_path_costs[j] == lowest and row4col[j] == -1):
            index = it
        lowest = min(lowest, shortest_path_costs[j])

        return remaining, min_value, cost, i, u, v, shortest_path_costs, path, lowest, row4col, index
    ...
    def _find_short_augpath_while_body(self, val):
        ...
        for it in range(num_remaining):
            remaining, min_value, cost, current_row, u, v, shortest_path_costs, path, lowest, row4col, index = self._find_short_augpath_while_body_inner_for(it, (remaining, min_value, cost, current_row, u, v, shortest_path_costs, path, lowest, row4col, index))
        ...
        if sink == -1:
            remaining, index, row4col, sink, current_row, SC, num_remaining = self._find_short_augpath_while_body_tail((remaining, index, row4col, sink, current_row, SC, num_remaining))

        return cost, u, v, path, row4col, current_row, min_value, num_remaining, remaining, SR, SC, shortest_path_costs, sink
    ...
    def _find_augmenting_path(self, cost, u, v, path, row4col, current_row):
        ...
        while self._find_short_augpath_while_cond((cost, u, v, path, row4col, current_row, min_value, num_remaining, remaining, SR, SC, shortest_path_costs, sink)):
            cost, u, v, path, row4col, current_row, min_value, num_remaining, remaining, SR, SC, shortest_path_costs, sink = self._find_short_augpath_while_body((cost, u, v, path, row4col, current_row, min_value, num_remaining, remaining, SR, SC, shortest_path_costs, sink))
    ...
    def _lsa_body(self, cost, u, v, path, row4col, col4row, current_row):
        sink, min_value, SR, SC, shortest_path_costs, path = self._find_augmenting_path(cost, u, v, path, row4col, current_row)
        ...
        breakvar = False
        while self._augment_previous_while_cond((path, sink, row4col, col4row, current_row, breakvar)):
            path, sink, row4col, col4row, current_row, breakvar = self._augment_previous_while_body((path, sink, row4col, col4row, current_row, breakvar))
