...
class solver_hungarian(Solver):
    ...
    def hungarian_algorithm(self, cost):
        ...
        # Initialize dual variables u and v to zero
        ...
        def find_augmenting_path(current_row):
            ...
            while sink == -1:
                index = -1
                lowest = np.inf
                visited_rows[current_row] = True  # Mark the current row as visited
                ...
                for it in range(num_remaining):
                    ...
                    if (shortest_path_costs[j] < lowest) or (shortest_path_costs[j] == lowest and row_to_col[j] == -1):
                        index = it
        ...
        for current_row in range(n):
            sink, min_value, visited_rows, visited_columns, shortest_path_costs = find_augmenting_path(current_row)

            # Update the dual variables u and v
            u[current_row] += min_value
            mask = visited_rows & (np.arange(n) != current_row)
            u += mask * (min_value - shortest_path_costs[col_to_row])
