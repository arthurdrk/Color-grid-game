...
def main():
    ...
    grid = Grid.from_file(os.path.join(data_path, selected_grid))
    solver = Solver(grid)
    ...
    while True:
        ...
        if not any(pair_is_valid(pair, solver.pairs) for pair in grid.all_pairs()):
            if not game_over:
                game_over = True
...
def pair_is_valid(pair, existing_pairs):
